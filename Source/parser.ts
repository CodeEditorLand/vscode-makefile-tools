...
async function parseAnySwitchFromToolArguments(
	...
	let parseCompilerArgsScriptFile: string =
		util.parseCompilerArgsScriptFile();
...
async function currentPathAfterCommand(
	...
	if (
		...
		newCurrentPath = await util.makeFullPath(
			line.slice(3),
			lastCurrentPath,
		);
	...
	async function doParsingChunk(): Promise<void> {
		...
		while (index < numberOfLines && chunkIndex <= chunkSize) {
			...
			let compilerTool: ToolInvocation | undefined =
				await parseLineAsTool(line, compilers, currentPath);
			...
			if (compilerTool) {
					...
					if (tempFragments.length > 1) {
						for (let i: number = 1; i < tempFragments.length; i++) {
							compilerFragments.push(
								"-D" +
									tempFragments[i].trim().split("/\s+/")[0],
							);
						}
					}
				}
				...
				forcedIncludes = await util.makeFullPaths(
					forcedIncludes,
					currentPath,
				);
				...
				let targetArchitecture: util.TargetArchitecture =
					getTargetArchitecture(compilerTool.arguments);
				...
				if (isSwitchPassedInArguments(compilerTool.arguments, ["TP"])) {
					...
					isSwitchPassedInArguments(compilerTool.arguments, ["TC"])
				) {
					language = "c";
				}
				...
				if (language) {
					...
					let standard: util.StandardVersion | undefined =
						parseStandard(cppToolsVersion, standardStr, language);
...
export function parseStandard(
	...
	if (!std) {
		...
		logger.message(
			localize("unknown.language", "Unknown language"),
			"Normal",
		);
	}
