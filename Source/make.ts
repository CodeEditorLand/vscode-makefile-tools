...
export async function buildTarget(
	...
	if (extension.getState().configureDirty) {
		...
		if (configuration.getConfigureAfterCommand()) {
			configureExitCode = await configure(
				TriggeredBy.configureBeforeBuild,
			);
	...
	try {
		return await vscode.window.withProgress(
			...
			async (progress, cancel) => {
				...
				let buildElapsedTime: number =
					util.elapsedTimeSince(buildStartTime);
...
function analyzeConfigureSubphases(stats: ConfigureSubphasesStatus): number {
	...
	if (
		...
		return stats.loadFromCache?.retc ===
...
export async function configure(
	...
	if (configuration.getAlwaysPreConfigure()) {
		preConfigureExitCode = await preConfigure(
			TriggeredBy.alwaysPreconfigure,
		);
	...
	try {
		...
		if (retc === ConfigureBuildReturnCodeTypes.success) {
			logger.message(
				localize("configure.succeeded", "Configure succeeded."),
			);
		} else {
		...
		let newBuildTarget: string | undefined =
			configuration.getCurrentTarget();
		...
		if (getConfigureIsClean()) {
			...
			configuration.setLaunchTargets(
				util.sortAndRemoveDuplicates(
					configuration
						.getLaunchTargets()
						.concat(launchConfigurationsStr),
				),
			);
		}
				...
				await util.scheduleTask(() => {
					...
					extension
						...
						.setCustomConfigurationProvider({
							workspaceBrowse:
								configurationCache.customConfigurationProvider
									.workspaceBrowse,
...
export async function doConfigure(
	...
	if (
		subphaseStats.parseLaunch.retc ===
		ConfigureBuildReturnCodeTypes.cancelled
	) {
		return subphaseStats;
	}
