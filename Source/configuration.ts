...
export async function readBuildLog(): Promise<boolean> {
	...
	if (buildLog) {
		...
		logger.message(
			localize(
				"build.log.defined.at",
				'Build log defined at "{0}"',
				buildLog,
			),
		);
...
export async function readLoggingLevel(): Promise<void> {
	...
	logger.message(
		localize("logging.level", "Logging level: {0}", loggingLevel),
	);
}
...
export async function readAlwaysPreConfigure(): Promise<void> {
	alwaysPreConfigure =
		await util.getExpandedSetting<boolean>("alwaysPreConfigure");
	logger.message(
		localize(
...
export async function readConfigurationCachePath(): Promise<void> {
	...
	if (configurationCachePath) {
		...
		if (extensionOutputFolder && !parsePath.dir) {
			...
			configurationCachePath = util.resolvePathToRoot(
				configurationCachePath,
			);
		}
...
export async function getCommandForConfiguration(
	...
	if (configurationMakefile) {
		...
		if (util.checkDirectoryExistsSync(configurationMakefile)) {
			let makeFileTest: string = path.join(
...
export async function readMakefileConfigurations(): Promise<void> {
	...
	let unnamedConfigurationNames: string[] = makefileConfigurations.map(
		(k) => {
			return k.name;
		},
	);
...
export async function readPhonyOnlyTargets(): Promise<void> {
	phonyOnlyTargets =
		await util.getExpandedSetting<boolean>("phonyOnlyTargets");
	logger.message(
		localize(
			"only.phony.targets",
			"Only .PHONY targets: {0}",
			phonyOnlyTargets,
		),
	);
}
...
export async function readBuildBeforeLaunch(): Promise<void> {
	buildBeforeLaunch =
		await util.getExpandedSetting<boolean>("buildBeforeLaunch");
	logger.message(
		localize(
			"build.before.launch",
		...
		if (
			...
			if (updatedBuildLog !== buildLog) {
				...
				let currentMakefileConfiguration:
					...
					| undefined = makefileConfigurations.find((k) => {
			...
			if (updatedExtensionLog) {
				...
				if (extensionOutputFolder && !parsePath.dir) {
					...
					updatedExtensionLog =
						util.resolvePathToRoot(updatedExtensionLog);
				}
			}
			...
			updatedSettingsSubkeys.forEach(async (subKey) => {
				...
				try {
					...
					telemetryProperties = await telemetry.analyzeSettings(
						settingObj,
						key,
						util.thisExtensionPackage().contributes.configuration
							.properties[key],
						false,
						telemetryProperties,
					);
			...
			if (
				telemetryProperties &&
				util.hasProperties(telemetryProperties)
			) {
...
export async function selectTarget(): Promise<void> {
	...
	const chosen: string | undefined =
		await vscode.window.showQuickPick(buildTargets);
...
export async function selectLaunchConfiguration(): Promise<void> {
	...
	launchConfigurations.forEach((launchConfiguration) => {
		if (util.checkFileExistsSync(launchConfiguration.binaryPath)) {
			launchTargetsNames.push(
				launchConfigurationToString(launchConfiguration),
			);
		}
	});
